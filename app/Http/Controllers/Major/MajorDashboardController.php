...
class MajorDashboardController extends Controller
{
    public function index()
    {
        ...
        // ── Congés sans avis du Major (action requise) ─────────────────
        $congesSansAvisCount = Demande::whereHas('agent', fn ($q) => $q->where('id_service', $serviceId))
            ->where('type_demande', 'Conge')
            ->whereIn('statut_demande', ['En_attente', 'Approuvé'])
            ->whereNull('avis_major')
            ->count();

        $congesSansAvis = Demande::whereHas('agent', fn ($q) => $q->where('id_service', $serviceId))
            ->where('type_demande', 'Conge')
            ->whereIn('statut_demande', ['En_attente', 'Approuvé'])
            ->whereNull('avis_major')
        ...
        return view('major.dashboard', compact(
            ...
            'congesSansAvisCount', 'congesSansAvis',
            'planningsEnCours', 'absencesMois',
            'dernieresAbsences', 'lignesSemaine',
            'absenteisme6Mois', 'labelsAbsences',
            'postesLabels', 'postesData'
        ));
    }
}
